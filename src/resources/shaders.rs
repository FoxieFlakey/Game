...
use crate::{error, registries::util, registry::Registry, runtimes, states, util::identifier::Identifier};
...
async fn load_list(shaders: &[Shader]) -> anyhow::Result<Registry<wgpu::ShaderModule>> {
    util::build_registry(shaders.into_iter(), |info| async move {
        ...
        let (module, future) = runtimes::compute::exec(move || {
            let error_future = states::main_dev::get().push_error_scope(wgpu::ErrorFilter::Validation);
            let module = states::main_dev::get()
...
pub async fn load() -> anyhow::Result<Registry<wgpu::ShaderModule>> {
    load_list(&[
        Shader::new("colored_rectangle", include_str!("colored_rectangle_shader.wgsl"))
    ]).await
}

pub async fn early_load() -> anyhow::Result<Registry<wgpu::ShaderModule>> {
    load_list(&[
        Shader::new("loading_icon", include_str!("loading_screen.wgsl"))
    ])
    .await
}
