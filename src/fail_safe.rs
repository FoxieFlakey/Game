use std::{error::Error, pin::Pin, rc::Rc, sync::atomic::{AtomicBool, AtomicU32, Ordering}, time::{Duration, Instant}};
...
pub fn init() -> Result<(), ErrorWithContext<StringError>> {
    ...
    unsafe { signal_hook::low_level::register(signal_hook::consts::SIGINT, handle_sig_hardquit) }
        .map_err(|e| ErrorWithContext::with_message("Cannot install SIGINT handler", Box::new(e) as Box<dyn Error>))?;
    
    // SAFETY: The handler is only calls async-signal-safe functions and does not panics
    unsafe { signal_hook::low_level::register(signal_hook::consts::SIGTERM, handle_sig_term) }
        .map_err(|e| ErrorWithContext::with_message("Cannot install SIGTERM handler", Box::new(e) as Box<dyn Error>))?;

    Ok(())
}

pub async fn fail_safe_guard(main: impl Fn(Box<dyn Fn() -> Pin<Box<dyn Future<Output = ()>>>>) -> Pin<Box<dyn Future<Output = Result<(), ErrorWithContext<dyn Error + 'static>>>>>) -> Result<(), ErrorWithContext<dyn Error + 'static>> {
