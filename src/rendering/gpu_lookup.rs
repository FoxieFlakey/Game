...
use crate::{info, util::{self, ErrorWithContext}};
...
pub async fn get_gpus() -> &'static Vec<wgpu::Adapter> {
	if GPU_LIST.get().is_none() {
		let adapters = super::WGPU.enumerate_adapters(wgpu::Backends::PRIMARY | wgpu::Backends::SECONDARY).await;
...
pub fn find_gpu(compatible_with: &wgpu::Surface<'_>) -> Result<&'static wgpu::Adapter, ErrorWithContext<GPULookupError>> {
	Ok(
		GPU_LIST.get()
