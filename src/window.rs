...
impl Window {
    pub fn new(builder: &sdl3::video::WindowBuilder) -> Result<Self, ErrorWithContext<CreateWindowError>> {
        ...
        Ok(Window {
            surface: unsafe {
                rendering::WGPU.create_surface_unsafe(wgpu::SurfaceTargetUnsafe::RawHandle {
    ...
    pub fn get_id(&self) -> NonZero<u32> {
        ...
        if ret == 0 {
            ...
            panic!("Cannot get window ID, should not happen: {}", sdl3::get_error());
        }
    ...
    pub fn get_size(&self) -> (u32, u32) {
        ...
        if ret == false {
            // This function is consired "important" so it shouldnt fail, if it fails we panic
            panic!("Cannot retrieve window size, should not happen: {}", sdl3::get_error());
        }
        (w as u32, h as u32)
    }
